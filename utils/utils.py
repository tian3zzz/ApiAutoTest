...
# 获取测试数据
def get_yaml_test_data(testCases):
    test_data = {}
    for key, value in testCases['testCases'].items():
        test_data[key] = value
    return test_data
...
# 获取断言数据后进行断言
def get_assert_data(testCases, response):
    assert_data = get_yaml_assert(testCases)
